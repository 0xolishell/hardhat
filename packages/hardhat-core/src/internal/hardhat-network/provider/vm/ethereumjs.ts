...
import { BlockBuilder } from "@nomicfoundation/ethereumjs-vm/dist/buildBlock";
...
import { BlockBuilder as IBlockBuilder } from "./vm-adapter";
...
class EthereumJsBlockBuilder implements IBlockBuilder {
  constructor(private _blockBuilder: BlockBuilder) {}

  public getGasUsed(): bigint {
    return this._blockBuilder.gasUsed;
  }

  public addTransaction(tx: TypedTransaction): Promise<RunTxResult> {
    return this._blockBuilder.addTransaction(tx);
  }

  public build(): Promise<Block> {
    return this._blockBuilder.build();
  }

  public revert(): Promise<void> {
    return this._blockBuilder.revert();
  }
}

// FVTODO implement interface
export class EthereumJSAdapter /* implements VMAdapter */ {
  ...
  public async dryRun(
    ...
    try {
      ...
      await this.revertToStateRoot(initialStateRoot);
    }
  }
  ...
  public async revertToStateRoot(stateRoot: Buffer): Promise<void> {
  ...
  public async buildBlock(
    ...
    return new EthereumJsBlockBuilder(blockBuilder);
  }
  ...
  private _selectHardfork(blockNumber: bigint): string {
    if (
      ...
      return this.getCommon().hardfork() as HardforkName;
    }
