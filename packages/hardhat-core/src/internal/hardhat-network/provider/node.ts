...
Hardhat Network's forking functionality only works with blocks from at least spuriousDragon.`
  ...
  private constructor(
    ...
    this._vmTracer = new VMTracer(this._vm as any, false);
    ...
    // FVTODO this should be (temporarily) done in the RethnetAdapter
    // constructor
  ...
  public async revertToSnapshot(id: number): Promise<boolean> {
    ...
    await this._vm.revertToStateRoot(
  ...
  private async _mineBlockWithPendingTxs(
    ...
    const blockBuilder = await this._vm.buildBlock(parentBlock, headerData);
    ...
    try {
      ...
      while (
        ...
        if (
          ...
          // FVTODO replace with RethnetAdapter's block builder
  ...
  private async _runInBlockContext<T>(
    ...
    try {
      ...
      await this._vm.restoreBlockContext(currentStateRoot);
    }
  }
  ...
  private async _runTxAndRevertMutations(
    ...
    // FVTODO move inside RethnetAdapter
    // const rethnetTx = ethereumjsTransactionToRethnet(tx);
  ...
  public isEip1559Active(blockNumberOrPending?: bigint | "pending"): boolean {
    ...
    return this._vm.getCommon().gteHardfork("london");
  }
  ...
  private _selectHardfork(blockNumber: bigint): string {
    if (
      ...
      return this._vm.getCommon().hardfork() as HardforkName;
    }
